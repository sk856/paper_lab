...
class SkillsCenterPanel:
    ...
    def _render_skill_list_item(self, item):
        ...
        shell = tk.Frame(self.list_inner, bg=self.list_inner.cget('bg'))
        shell.pack(fill=tk.X, pady=(0, 10))
        body = RoundedFrame(
            shell,
            bg=shell.cget('bg'),
            fill=COLORS['accent_soft'] if is_selected else COLORS['card_bg'],
            outline=COLORS['primary'] if is_selected else COLORS['card_border'],
            radius=14,
            cursor='hand2',
        )
        body.pack(fill=tk.X)

        body_bg = COLORS['accent_soft'] if is_selected else COLORS['card_bg']
        title_row = tk.Frame(body.body, bg=body_bg)
        ...
        bind_adaptive_wrap(desc_label, body.body, padding=24, min_width=220)
