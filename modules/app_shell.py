...
class SmartPaperTool:
    ...
    def __init__(self):
        ...
        self._page_switch_job = None
        self._page_on_show_job = None
        self._page_switching_to = None
    ...
    def _initialize_window_theme(self):
        theme_mode = self.config_mgr.get_setting('theme_mode', 'dark')
    ...
    def _get_startup_loading_palette(self):
        """根据已保存主题为启动加载窗选择配色。"""
        try:
            theme_mode = ConfigManager(BASE_DATA_DIR).get_setting('theme_mode', 'dark')
            resolved = resolve_theme_mode(theme_mode)
        except Exception:
            resolved = 'dark'
        return THEMES.get(resolved, THEMES['dark']).copy()
    ...
    def _show_loading_screen(self):
        ...
        tk.Frame(card, bg=accent_color, width=6).pack(side=tk.LEFT, fill=tk.Y)
    ...
    def _build_top_nav(self):
        ...
        self.user_row_pad_left = 14
        self.user_row_pad_right = 14
        ...
        top_pad = 14 if self.window_chrome else 18
        pack_kwargs = {
            'fill': tk.X,
            'padx': 32,
            'pady': (top_pad, 0),
        ...
        for page_id, label in nav_items:
            ...
            border = tk.Frame(shell, bg=COLORS['nav_bg'], bd=0, highlightthickness=0)
    ...
    def _apply_top_nav_spacing(self):
        ...
        outer_pad = int(round(30 + 14 * progress))
        nav_gap = int(round(8 + 6 * progress))
        tool_gap = int(round(6 + 4 * progress))
        user_gap = int(round(8 + 6 * progress))
    ...
    def _measure_top_nav_canvas_size(self, label):
        ...
        canvas_width = max(text_width + 42, 104)
        canvas_height = max(text_height + 20, 46)
        if canvas_width % 2 != 0:
            canvas_width += 1
    ...
    def _render_top_nav_canvas(self, canvas):
        if canvas is None:
            return
        nav_outline = COLORS['card_border']
        nav_normal_fill = COLORS['surface_alt']
        nav_active_fill = COLORS['nav_active']
        try:
            req_width = int(float(canvas.cget('width')))
            req_height = int(float(canvas.cget('height')))
        except Exception:
        ...
        canvas.delete('all')
        canvas.configure(bg=COLORS['nav_bg'], highlightthickness=0, bd=0)
        radius = max(12, min(width, height) // 2 - 2)
        self._create_round_rect(
            canvas,
            inset,
            inset,
        ...
        canvas.create_text(
            ...
            center_y,
            text=label,
            fill=COLORS['pill_active_fg'] if is_active else COLORS['text_sub'],
            font=FONTS['nav'],
            anchor='center',
            justify='center',
        )

    def _create_round_rect(self, canvas, x1, y1, x2, y2, radius, **kwargs):
    ...
    def _show_page(self, page_id, *, invoke_on_show=True):
        ...
        if page_id == self.current_page_id and page_id in self.pages:
            return
        if self._page_switch_job is not None:
            try:
                self.root.after_cancel(self._page_switch_job)
            except tk.TclError:
                pass
            self._page_switch_job = None
        if self._page_on_show_job is not None:
            try:
                self.root.after_cancel(self._page_on_show_job)
            except tk.TclError:
                pass
            self._page_on_show_job = None
        self._page_switching_to = page_id
        ...
        if page_id in self.pages:
            ...
            self._prepare_page_switch_surface(page_id)
            self._page_switch_job = self.root.after(
                1,
    ...
    def _ensure_and_show_page(self, page_id, *, invoke_on_show=True):
        self._page_switch_job = None
        if self._page_switching_to != page_id:
            return
        page = self._ensure_page(page_id)
        if not page:
            return
    ...
    def _prepare_page_switch_surface(self, page_id):
        ...
        if hasattr(self, 'content_view'):
            try:
                self.content_view.canvas.yview_moveto(0)
                self.content_view.canvas.configure(bg=COLORS['bg_main'])
                self.content_view.inner.configure(bg=COLORS['bg_main'])
                self.content_view.canvas.configure(scrollregion=(0, 0, self.content_view.canvas.winfo_width(), 1))
                self.content_view.canvas.update_idletasks()
            except tk.TclError:
                pass
    ...
    def _switch_to_page(self, page_id, *, invoke_on_show=True):
        ...
        if hasattr(self, 'content_view'):
            ...
            try:
                self.content_view._apply_layout_refresh()
            except Exception:
                pass
        if invoke_on_show and hasattr(page, 'on_show'):
            self._page_on_show_job = self.root.after_idle(lambda pid=page_id: self._invoke_page_on_show(pid))
    ...
    def _show_theme_menu(self):
        ...
        current_mode = self.config_mgr.get_setting('theme_mode', 'dark')
    ...
    def _show_settings(self):
        ...
        theme_var = tk.StringVar(value=theme_display.get(self.config_mgr.get_setting('theme_mode', 'dark'), '深色模式'))
        ...
        model_test_shell = tk.Frame(advanced_page, bg=advanced_page.cget('bg'), bd=0, highlightthickness=0)
        ...
        model_test_card.pack(fill=tk.X)
        ...
        parameter_shell = tk.Frame(advanced_page, bg=advanced_page.cget('bg'), bd=0, highlightthickness=0)
        ...
        billing_shell = tk.Frame(advanced_page, bg=advanced_page.cget('bg'), bd=0, highlightthickness=0)
        ...
        webdav_form_shell = tk.Frame(backup_page, bg=backup_page.cget('bg'), bd=0, highlightthickness=0)
        ...
        current_theme = theme_display.get(self.config_mgr.get_setting('theme_mode', 'dark'), '深色模式')
        ...
        def save_settings():
            warnings = []
            theme_mode = theme_reverse.get(theme_var.get(), 'dark')
    ...
    def _apply_dwm_titlebar_color(self, resolved_mode):
        ...
        try:
            ...
            if not hwnd:
                hwnd = self.root.winfo_id()
            theme = THEMES.get(resolved_mode, THEMES['dark'])
    ...
    def _apply_theme(self, theme_mode, refresh=True):
        ...
        self._set_status(f'主题已切换为{ {"light": "浅色模式", "dark": "深色模式"}.get(resolved, "深色模式") }')
        self.config_mgr.save()
    ...
    def _refresh_shell_styles(self):
        ...
        for border in self.nav_button_borders:
            border.configure(bg=COLORS['nav_bg'])
        ...
        if getattr(self, 'user_box', None):
            self.user_box.configure(bg=COLORS['nav_bg'])
            if getattr(self, 'user_inner', None):
                self.user_inner.configure(bg=COLORS['nav_bg'])
            if getattr(self, 'user_content', None):
                self.user_content.configure(bg=COLORS['nav_bg'])
            if getattr(self, 'user_canvas', None):
                self.user_canvas.configure(bg=COLORS['nav_bg'])
                self._render_user_profile_canvas(self.user_canvas)
